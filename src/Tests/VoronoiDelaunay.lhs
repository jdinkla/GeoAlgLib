...
>	    when (DC `elem` as) (
>	      if m==N then outDC sc (_scc_ "delDC" (delaunay ps))
>		      else outDC sc (_scc_ "delDC_DE" (delaunay qs)))
>	    when (Vor `elem` as) (
>	      if m==N then outVor sc ps (_scc_ "vorDC" (voronoi ps))
>		      else outVor sc qs (_scc_ "vorDC_DE" (voronoi qs)))
>           when (DAG `elem` as) (
>	      if m==N then outDAG sc (_scc_ "delDAG" (DDAG.triangles (DDAG.delaunay ps)))
>		      else outDAG sc (_scc_ "delDAG_DE" (DDAG.triangles (DDAG.delaunay qs))))
>	    putStrLn "\n end"
>
